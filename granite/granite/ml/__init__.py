...
from .estimators import GetRegModels, GetClassModels

from ..plotting.ml import ROCCurve, FeatureDependence, TrainingPlot, ConfusionMatrix
...
class Experiment():
    ...
    def plot_training_scores(self, model, fig=None, ax=None):
        ...
        TrainingPlot(self.models[model].total_time,
    ...
    def plot_confusion_matrix(self, model, train=True, fig=None, ax=None):
        ...
        ConfusionMatrix(y, ypred, fig=fig, ax=ax)
    ...
    def plot_roc_curve(self, model, train=True, fig=None, ax=None):
        ...
        ROCCurve(y, probs, fig=fig, ax=ax)
    ...
    def plot_feature_dependence(self, feature, model=None, cat=None, train=True, bins=None, ylims=None, fig=None, ax=None):
        ...
        FeatureDependence(X, y, feature, dtype, model=model, cat=cat, bins=bins, ylims=ylims, fig=fig, ax=ax)
...
class GraniteProject():
    ...
    def save_plot(self, style, options, index):
        if index > len(self.plots) - 1:
            self.plots.append([style, options])
        else:
            self.plots[index] = [style, options]
    ...
    def remove_plot(self, index):
        if index < len(self.plots):
            ...
            print('Removing plot failed: index out of range')
