...
class ItemsView(QtWidgets.QWidget):
    ...
    """
    def __init__(self, frame, has_button=True, has_remove=False, *args, **kwargs):
    ...
    def AddItem(self, widget, index=None):
        ...
        """
        if index is None or index > len(self.widgets) - 1:
            # Add a new item
            ...
            # Stretch end
            self.grid.setRowStretch(len(self.widgets)-1, 0)
            self.grid.setRowStretch(len(self.widgets), 1)
        else:
        ...
        if self.has_button:
            widget.button.clicked.connect(lambda x=index: self.SelectItem(x))
        
        if self.has_remove:
            widget.removeButton.clicked.connect(lambda x=index: self.RemoveItem(x))
    ...
    def RemoveItem(self, index):
        ...
        msgBox.setText('')
        msgBox.setInformativeText('Would you like to permanently delete this item?')
        ...
        self.frame.RemoveItem(index)
        
        # Remove icon from the list
        self.widgets[index].setParent(None)
        self.widgets.pop(index)
        
        # Update other widgets indices
        for i in range(index, len(self.widgets)):
            self.grid.addWidget(self.widgets[i], i, 0)
            
            if self.has_button:
                self.widgets[i].button.clicked.disconnect()
                self.widgets[i].button.clicked.connect(lambda x=index: self.SelectItem(x))
                
            if self.has_remove:
                self.widgets[i].removeButton.clicked.disconnect()
                self.widgets[i].removeButton.clicked.connect(lambda x=index: self.RemoveItem(x))
                
        # Move add button
        self.grid.addWidget(self.addButton, len(self.widgets), 0)
        
        # Move stretched row to new end row
        self.grid.setRowStretch(len(self.widgets)+1, 0)
        self.grid.setRowStretch(len(self.widgets), 1)
...
class BoxFrame(QtWidgets.QFrame):
    ...
    def __init__(self, title=None, side=None, *args, **kwargs):
        ...
        frame.setObjectName('foreground')
